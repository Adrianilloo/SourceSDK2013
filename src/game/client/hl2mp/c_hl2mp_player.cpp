...
{
	return ( !(m_Local.m_bDucked && FBitSet(GetFlags(), FL_ANIMDUCKING)) && (GetWaterLevel() != 3) );
}
...
{
	bool bWantSprint = ( CanSprint() && IsSuitEquipped() && (m_nButtons & IN_SPEED) );

	if ( IsSprinting() != bWantSprint && FBitSet(mv->m_nButtons ^ mv->m_nOldButtons, IN_SPEED) )
	{
		if ( bWantSprint )
		{
			StartSprinting();
	...
	if ( IsSuitEquipped() )
	{
		bWantWalking = ( mv->m_nButtons & IN_WALK ) && !IsSprinting() && !(mv->m_nButtons & IN_DUCK);
	}

	if( IsWalking() != bWantWalking )
	{
		if ( bWantWalking )
		{
			StartWalking();
